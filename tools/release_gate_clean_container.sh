...
UCODE_SRC=${CW_GATE_UCODE:-$HOME/DR2CW-troubleshooting/ucode-dumps}
UCODE_ONE=$(ls "$UCODE_SRC"/ps_*.ucode 2>/dev/null | head -1)
[ -n "$UCODE_ONE" ] || { echo "FAIL: no ps_*.ucode under $UCODE_SRC (set CW_GATE_UCODE)" >&2; exit 1; }
UCODE_DIR=$(mktemp -d)
...
PKGFILE=${CW_GATE_PACKAGE:-$(find "$(cd "$(dirname "$0")/.." && pwd)/assets/package" \
    -type f -size +100M 2>/dev/null | head -1)}
[ -n "$PKGFILE" ] && [ -f "$PKGFILE" ] \
    || { echo "FAIL: no STFS package under assets/package (set CW_GATE_PACKAGE)" >&2; exit 1; }

# The overlay oracle (Case Zero release-github §0.4, retargeted): the container
# regenerates the KB/M prompt overlay with the C++ generator, and its fecmn.tex
# must hash to what the PYTHON reference produced in the dev tree — the same
# byte-identity contract the stfs extract carries. Required, not skipped-if-absent
# (regenerate with `python3 tools/gen_kbm_icons.py` if this refuses). Case West has
# no game_patched layer — its Visuals panel is a code hook, not a repacked asset.
OVERLAY_REF="$(cd "$(dirname "$0")/.." && pwd)/assets/game_kbm/data/frontend/fecmn.tex"
[ -f "$OVERLAY_REF" ] \
    || { echo "FAIL: no $OVERLAY_REF — run tools/gen_kbm_icons.py first" >&2; exit 1; }
...
if CW_ROOT=/w $APP/cw_runtime --gen-overlays >/tmp/overlay.log 2>&1; then
    ...
    for f in /w/assets/game_kbm/data/frontend/fecmn.tex \
             ...
             /w/assets/game_kbm/data/frontend/ingame.big \
    ...
    # the C++ generator in the container must produce the same fecmn.tex, hash
    # for hash, that gen_kbm_icons.py produced on the host.
    got=$(sha256sum /w/assets/game_kbm/data/frontend/fecmn.tex | cut -d' ' -f1)
    if [ "$got" != "$CW_GATE_OVERLAY_SHA" ]; then
        echo "    overlay HASH MISMATCH: container $got vs Python reference $CW_GATE_OVERLAY_SHA"
        ok=0
    fi
    [ "$ok" = 1 ] && echo "    overlay-gen OK (fecmn.tex matches the Python reference: $got)"
else
