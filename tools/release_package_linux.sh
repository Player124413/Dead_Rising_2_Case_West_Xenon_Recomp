...
[ -x "$BUILD/cw_runtime" ] || fail "no executable at $BUILD/cw_runtime — configure a
  release tree first:
    cmake -S runtime -B runtime/build-release -G Ninja -DCMAKE_BUILD_TYPE=Release \\
        -DCW_FFMPEG_PREFIX=$ROOT/thirdparty/ffmpeg-lgpl
...
if ldd "$BUILD/cw_runtime" | grep -q 'libavcodec.*=> */\(usr/\)\?lib'; then
  ...
  --enable-gpl and must not be shipped with a PolyForm repo (release-plan §4).
    ...
    cmake -S runtime -B $BUILD -DCW_FFMPEG_PREFIX=$ROOT/thirdparty/ffmpeg-lgpl ..."
fi
...
if [ -f "$STAGE/lib/libSDL2-2.0.so.0" ] \
  ...
  Build real SDL2 and point the release tree at it:
    tools/build_sdl2.sh
    cmake -S runtime -B $BUILD -DCW_SDL2_PREFIX=$ROOT/thirdparty/sdl2 ..."
fi
...
DXC_SRC=${CW_DXC_LIB:-$HOME/GithubRepo/XenosRecomp/thirdparty/dxc-bin/lib/x64/libdxcompiler.so}
[ -f "$DXC_SRC" ] || fail "no libdxcompiler.so at $DXC_SRC — the shipped shader
  ...
  Point CW_DXC_LIB at a libdxcompiler.so (XenosRecomp's thirdparty/dxc-bin has one)."
...
Put your own copy of the Dead Rising 2: Case West XBLA package in this directory.
...
and it is about 1.2 GB. Copying the whole 58410B00 folder in here works too — the
...
# Harvest a fresh one from %LOCALAPPDATA%/cw-recomp (or ~/.cache/cw-recomp) after a
...
# RELEASE DEFAULTS (part 85). Every dev recipe says CW_VKDRAW=1 out loud and the
