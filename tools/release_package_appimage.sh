...
#                             exists), then execs usr/cw_runtime with the player's args
#   cw_runtime.desktop, cw_runtime.png, .DirIcon
#                             what desktop integration tools read; the icon is OUR art
#                             (tools/release/make_icon.py) — no Capcom byte ships
#   usr/                      the staged bundle minus assets/: cw_runtime, lib/ (SDL2,
#                             ffmpeg, libstdc++, libdxcompiler.so), prewarm.keys,
#                             vs_recipes.bin, kbm_chips/, cw_defaults.env, README,
...
STAGE=${1:-$ROOT/dist/CaseWestRecomp}
OUT=${2:-$ROOT/dist}
NAME=CaseWestRecomp
APPDIR=$OUT/$NAME.AppDir
IMG=$OUT/$NAME-linux-x86_64.AppImage
RUNTIME=${CW_APPIMAGE_RUNTIME:-$ROOT/thirdparty/appimage/runtime-x86_64}
RUNTIME_URL=https://github.com/AppImage/type2-runtime/releases/download/continuous/runtime-x86_64

fail() { echo "FAIL: $*" >&2; exit 1; }
[ -x "$STAGE/cw_runtime" ] || fail "no staged bundle at $STAGE — run tools/release_package_linux.sh first"
...
for entry in "$STAGE"/* "$STAGE"/.[!.]*; do
    ...
    case "$(basename "$entry")" in
        ...
        cw_runtime.log*|cw_diag.txt*) continue ;;   # a gate's leftovers (part 105)
    esac
...
#!/bin/sh
# AppRun for CaseWestRecomp. The runtime resolves its data root from $APPIMAGE (the
...
# first-run refusal points at a directory that exists. CW_ROOT, if set, wins and nothing
# is seeded.
HERE=$(dirname "$(readlink -f "$0")")
if [ -n "${APPIMAGE:-}" ] && [ -z "${CW_ROOT:-}" ]; then
    d=$(dirname "$APPIMAGE")/assets/package
    if [ ! -d "$d" ]; then
        mkdir -p "$d" 2>/dev/null && cp "$HERE/usr/PUT_YOUR_GAME_HERE.txt" "$d/" 2>/dev/null
    fi
fi
exec "$HERE/usr/cw_runtime" "$@"
SH
chmod +x "$APPDIR/AppRun"

cat > "$APPDIR/cw_runtime.desktop" <<'DESK'
[Desktop Entry]
Type=Application
Name=Dead Rising 2: Case West Recomp
...
Categories=Game;
DESK
[ -f "$ROOT/tools/release/icon/cw_runtime.png" ] || fail "no tools/release/icon/cw_runtime.png (python3 tools/release/make_icon.py)"
cp "$ROOT/tools/release/icon/cw_runtime.png" "$APPDIR/cw_runtime.png"
ln -sf cw_runtime.png "$APPDIR/.DirIcon"
