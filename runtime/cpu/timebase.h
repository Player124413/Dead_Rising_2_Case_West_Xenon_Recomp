...
inline constexpr uint64_t CW_TIMEBASE_HZ = 49875000ull;
...
// CW_DETERMINISTIC_CLOCK — a guest clock that advances a fixed quantum per PRESENTED
...
// of instrument as CW_FAKE_START_MS — it manufactures a condition rather than reporting
...
inline uint64_t guest_ticks()
{
    ...
    return uint64_t((__uint128_t(host_rdtsc()) * CW_TIMEBASE_HZ) / host_hz);
}
