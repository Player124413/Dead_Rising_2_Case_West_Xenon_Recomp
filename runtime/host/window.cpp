...
// CW_HAVE_SDL is set by CMake when the window is built (the default). The headless
// build is not a fallback the runtime can fall INTO — it is a configure-time choice
// (-DCW_WINDOW=OFF), and it says so on every startup, because "no window" and
// "window whose input is broken" are indistinguishable from a log otherwise.
// The three debug edges live OUTSIDE the CW_HAVE_SDL split, and deliberately.
//
...
// Defining them here lets `CW_FAKE_PRESS_SEQ`'s F2/F3/F4 entries work in a headless run and
// in a `-DCW_WINDOW=OFF` build, instead of a stub silently returning false — which is the
...
//
// `CW_VK_SNAP_DUMP` used to fire only at a fixed frame number (`CW_VK_SNAP_FRAME`, default
...
// keyboard, consumed elsewhere — so it works from `CW_FAKE_PRESS_SEQ` too and does not
...
{
    fprintf(stderr, "[host] built with -DCW_WINDOW=OFF: no window, no present seam and "
...
//
// `CW_INPUT_TRACE=1` has printed one line per pad state change since phase 3, and every
...
// keystrokes can be transcribed into a `CW_FAKE_PRESS_SEQ` recipe.
//
...
//
// It also DECODES, into exactly the vocabulary `CW_FAKE_PRESS_SEQ` accepts (A, START,
...
// now (the MOUSE CAMERA toggle is retired, 2026-09-05): capture follows window focus,
// so a pad player who never moves the mouse feeds zero deltas and nothing changes.
...
void PrintKeyMap()
{
    ...
    fprintf(stderr, "[host] mouse (always on): camera=right "
    ...
    if (g_keyboardFocus)
    {
        ...
        if (NativeKbm_Active() && !Settings_OverlayVisible())
        {
            ...
            if (g_relativeMouse)
            {
                ...
                if (mb & SDL_BUTTON(SDL_BUTTON_LEFT))
                    s.buttons |= XI_X;
                // RMB = LEFT trigger ONLY. Aim on the shipped pad is LT; R2 is
                // the FIRE side. MEASURED ON THIS IMAGE with CW_KBM_TRACE=1
                // CW_KBM_CMD_CENSUS=1 over a gameplay route, and read with the
                // guest's own combiner enum (C_AND=1, C_OR=3, native_kbm.cpp):
                //     PLAYER_FIRE_WEAPON     X PRESSED  OR  R2 PRESSED
                //     PLAYER_RAPID_FIRE_RT   X HELD     OR  R2 HELD
                //     PLAYER_THROW           X PRESSED AND  L2 HELD
                //     PLAYER_THROW_RT        R2 PRESSED AND L2 HELD
                // The part-8 import believed R2 was the aim source and held
                // BOTH triggers here (with a 70 ms stagger so the R2 edge would
                // not trip PLAYER_THROW_RT), which held the FIRE trigger
                // through every aim: RAPID_FIRE_RT's OR-with-HELD is satisfied
                // by R2 alone, so an automatic empties itself while aiming,
                // while a semi-auto — edge-fired — never shows it. Case Zero's
                // operator reproduced exactly that (their part 99, 684bff1:
                // assault rifle only, never on a physical pad holding LT).
                // LMB's X already covers everything R2 did: fire (X PRESSED),
                // auto fire (X HELD) and throw (X PRESSED while L2 held), and
                // with no mouse-side R2 edge at all the THROW_RT misfire the
                // stagger existed for is structurally impossible — so the
                // stagger goes with it. It also makes the epilogue camera's
                // "hold LT, press RB" prompt reachable instantly rather than
                // 70 ms after RMB goes down (part 9's keys 2/3 feed RB below).
        ...
        if (g_relativeMouse)
        {
            ...
            if (mb & SDL_BUTTON(SDL_BUTTON_LEFT))
                s.buttons |= XI_X;
            if (mb & SDL_BUTTON(SDL_BUTTON_RIGHT))
                s.leftTrigger = 255;   // LT = aim (R2 here is the FIRE trigger;
                                       // see the native block above)
            if (mb & SDL_BUTTON(SDL_BUTTON_MIDDLE))
                s.buttons |= XI_Y;
...
bool Host_RunLauncher()
{
    ...
    // 458 tall, and the arithmetic is the launcher's own layout, not a guess: the rows
    // start at y=96 and step 34, the drop-hint footer sits at 96 + rows*34 + 14 and the
    // control hint 22 below it, so NINE rows need 96 + 306 + 14 + 22 + 20 = 458. (The
    // sibling's is 492 because it has a tenth row, SKIP INTRO LOGOS, which this port
    // has not imported.) It was 420 here, sized for the SEVEN rows this launcher had
    // before MSAA and SUBTITLES were added — so the drop hint was drawn at y=416 in a
    // 420-tall window and the line telling a new player where to put their game file
    // was CLIPPED. Grow this whenever a row is added; the formula above says by how much.
    SDL_Window* win = SDL_CreateWindow("Dead Rising 2: Case West",
                                       SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                                       720, 458, SDL_WINDOW_ALLOW_HIGHDPI);
    if (!win)
        return true;
    ApplyGameIcon(win);
    ...
    // most is the one machine this project does not own. CW_LAUNCHER_PAD_TEST is a
    ...
    if (const char* t = getenv("CW_LAUNCHER_PAD_TEST"))
    {
        std::string cur;
        ...
        fprintf(stderr, "[launcher] CW_LAUNCHER_PAD_TEST: %zu synthetic pad inputs\n",
                padTest.size());
    }
    ...
    while (!play && !quit)
    {
        ...
        // per N=1..8 — each ID opens exactly its own str_XX.bcs, once, at file #40;
        // 3=de and 8=zh fall back to en, so they are not offered). ASCII names — this
        // 5x7 font has no accents or CJK; the in-GAME text is what gets localized.
        ...
        if (getenv("CW_LAUNCHER_PAD_TEST") && sel != lastSaid)
        {
            fprintf(stderr, "[padtest]   row %d %s = %s\n", sel, rows[sel].label,
                    rows[sel].value.empty() ? "(none)" : rows[sel].value.c_str());
            lastSaid = sel;
        }
        else if (getenv("CW_LAUNCHER_PAD_TEST") && rows[sel].value != lastValue)
        {
            fprintf(stderr, "[padtest]   row %d %s = %s\n", sel, rows[sel].label,
                    rows[sel].value.empty() ? "(none)" : rows[sel].value.c_str());
        }
...
bool Host_WindowInit()
{
    ...
    // run in this project is `timeout N ./cw_runtime`, and routing SIGTERM through our
    ...
    // `SDL_InitSubSystem(SDL_INIT_AUDIO)` from a guest thread — so a `CW_NO_WINDOW=1`
    ...
    // measured at `timeout 20` as exit 124 at 20 s with `CW_NO_AUDIO_OUT=1` and still
    ...
    if (getenv("CW_NO_WINDOW"))
    {
        fprintf(stderr, "[host] CW_NO_WINDOW=1 — headless: no window, no present, and "
...
// ALLOW_HIGHDPI on a phone and the swapchain would come out at a fraction of the panel.
    ...
    if (getenv("CW_VKDRAW"))
        fprintf(stderr, "[host] renderer requested (CW_VKDRAW): the window shows the "
                        "rendered frame once VkRenderer_Init succeeds — see the [vk] "
                        "lines for whether it did.\n");
    else
        fprintf(stderr, "[host] no renderer requested (CW_VKDRAW unset): the window "
...
void Host_WindowRun()
{
    ...
    for (;;)
    {
        ...
        {
            // ALWAYS ON (operator instruction, 2026-09-05, matching Case Zero's
            // release-session change): the MOUSE CAMERA toggle is retired and
            // capture follows FOCUS alone — released the moment a panel wants a
            // visible cursor or focus leaves, and a pad player who never moves the
            // mouse feeds zero deltas either way. State-change only:
            // SDL_SetRelativeMouseMode is not free.
            const bool wantRel =
                g_keyboardFocus &&
                !g_debugOverlayVisible.load(std::memory_order_acquire) &&
                !Settings_OverlayVisible();
