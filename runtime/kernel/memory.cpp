...
void Memory::Init()
{
    ...
    // whose absence is invisible for hours. CW_MEM_POISON_ALIAS=1 re-maps the middle
    ...
    if (const char* poison = getenv("CW_MEM_POISON_ALIAS"); poison && *poison != '0')
    {
        fprintf(stderr, "[mem] CW_MEM_POISON_ALIAS=%s — deliberately breaking the "
    ...
    fprintf(stderr, "[mem] physical aliasing OK: %08zX/%08zX/%08zX are one 512 MB "
                    "region (CW_MEM_POISON_ALIAS=1 is the positive control)\n",
    ...
    // field read is legal and common. When Case West starts faulting on null reads
    // that hardware tolerates, flip this with CW_NULL_PAGE_READABLE rather than
    ...
    const char* nullPage = getenv("CW_NULL_PAGE_READABLE");
